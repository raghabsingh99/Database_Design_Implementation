...
cardiology_code_set_table_name = 'CodeSetCardiology'
cardiology_code_set_table_ddl = f"""
        CREATE TABLE {cardiology_code_set_table_name} (
            ...
            CONSTRAINT FK_{cardiology_code_set_table_name}_Solution FOREIGN KEY (SolutionId) REFERENCES Solution(ID),
            CONSTRAINT FK_{cardiology_code_set_table_name}_CPTCode FOREIGN KEY (CPTCode) REFERENCES CPTCodes(CPTCode)
        );
        """
...
def insert_cardiology_code_set(
    ...
    insert_sql = f"""
        INSERT INTO 
        {cardiology_code_set_table_name} (
            SolutionId,
            ModalityId,
            HealthPlanId,
            CPTCode,
...
def process_cardiology_excel_and_insert_data(cursor: Cursor, current_year: str, health_plan_id: int, excel_file, solution_id, location: str):
    ...
    if check_if_health_plan_exists is not None:
        excel_data = read_excel_file(excel_file, 'CARDIOLOGY')
        for row in excel_data:
            ...
            procedure_number = row.get('procedure_#')
