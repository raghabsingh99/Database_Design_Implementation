...
def insert_sleep_code_set(cursor: Cursor, solution_id: int, cpt_code: str, grouper: str, quantity, 
                          qualifier, auth_time_frame, type_of_service, place_of_service, plans: dict, year, health_plan_id: int, location: str):
    if grouper is None:
        return None
    ...
    try:
        ...
        quantity = int(quantity) if quantity is not None and str(quantity).isdigit() else None
        health_plan_id = int(health_plan_id)
    except ValueError as e:
        print(f"Error converting fields to integer: {e}")
        raise

    # Ensure Location is a string and handle None
    ...
    """
    params = (
        solution_id, cpt_code, grouper, quantity, qualifier, auth_time_frame, type_of_service, place_of_service, plans, year, location, health_plan_id
    )
