...
def codeset_switch(cursor: Cursor, current_year: str, health_plan_id: int, excel_file, solution_id, solution: str):
    solution = solution.upper()
    if solution == "RAD":
        process_rad_excel_and_insert_data(current_year=current_year, cursor=cursor, excel_file=excel_file, health_plan_id=health_plan_id,solution_id=solution_id) 
        ...
        process_cardiology_excel_and_insert_data(current_year=current_year, cursor=cursor, excel_file=excel_file, health_plan_id=health_plan_id,solution_id=solution_id) 
        insert_file_log(cursor=cursor, file_path=excel_file)
    elif solution == "RBM" or solution == "RADIOLOGY":
        process_rbm_excel_and_insert_data(current_year=current_year, cursor=cursor, excel_file=excel_file, health_plan_id=health_plan_id,solution_id=solution_id) 
        insert_file_log(cursor=cursor, file_path=excel_file)
    else:
